...
| `x-all-setup-status/` | Setup status tracking — brand-level and per-product onboarding progress with checkboxes |
...
- Setup status tracking (per-brand folders with product-level progress)
